...
import { useDeleteVariation, useVariations } from '../../hooks/category.hook';
import Loader from '../../common/Loader';
import Search from '../../components/btn/Search';

const Variations = () => {
  const { data: variations, isPending } = useVariations();
  const { mutate: deleteVariationMutation, isPending: pending } =
    useDeleteVariation();
  if (isPending || pending) return <Loader />;
  return (
    <>
      <Breadcrumb pageName={`category list`} />
      <Search url="variations/list" />

      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="max-w-full overflow-x-auto">
          <Link
            to="/variations/create"
                ...
                <tr key={item._id}>
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <div className="flex items-center space-x-3.5">
                      <Link
                        to={`/variations/option/list/${item._id}`}
                      ...
                      <Link
                        className="hover:text-primary"
                        to={`/variations/option/create/${item._id}`}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                      ...
                      <button
                        className="hover:text-primary"
                        onClick={() => deleteVariationMutation(item._id)}
                      >
                        <svg
                          className="fill-current"
                          width="18"
