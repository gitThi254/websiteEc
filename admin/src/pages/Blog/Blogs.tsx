...
import Pagination_page from '../../components/Pagination_page_blog';
import SearchNotFound from '../../components/SearchNotFound';
import Empty from '../../components/Empty';

const Blogs = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const { data: blogs, isPending } = useBlogs(searchParams);
  const { data: category, isPending: pending } = useCategories();
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="max-w-full overflow-x-auto">
          ...
          {blogs?.blogs?.length !== 0 ? (
                    ...
                    <tr key={item._id}>
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="inline-flex rounded-full bg-success bg-opacity-10 py-1 px-3 text-sm font-medium text-success">
                          {item?.numViews} - {item?.likes?.length} -{' '}
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <div className="w-full h-[100px] border-2 relative">
                          <img
            ...
            <Empty title="Sản pha hàng trống" />
          )}
        </div>
      </div>
    </>
